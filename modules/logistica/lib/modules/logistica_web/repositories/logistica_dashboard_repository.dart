...
class LogisticaDashboardRepository {
  ...
  Future<LogisticaDashboardData> carregar() async {
    if (kIsWeb) {
      return _carregarWeb();
    }
  ...
  Future<LogisticaDashboardData> _carregarWeb() async {
    final servidorOnline = await apiClient.testarConexao();
    final viagensApi = await apiClient.buscarLogisticaViagens();
    final viagensDriverApi = await apiClient.buscarViagensMockadas();
    final motoristasApi = await apiClient.buscarLogisticaMotoristas();
    final veiculosApi = await apiClient.buscarLogisticaVeiculos();
    final pacientesApi = await apiClient.buscarLogisticaPacientes();
    ...
    final passageirosApi = <Map<String, dynamic>>[];
    for (final viagem in viagensOrigem) {
      final viagemId = _stringValue(viagem, ['id', 'viagem_id']);
      if (viagemId == null || viagemId.isEmpty) continue;
      passageirosApi.addAll(
        await apiClient.buscarLogisticaPassageiros(viagemId),
      );
    }
